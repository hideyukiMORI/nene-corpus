...
| 優先 | 項目 | メモ |
| --- | --- | --- |
| P2 | **ソース表・一覧の折り返し** | 長いソース名/タイトルで横スクロール必須になる問題。`break-words` 等で「ドキュメント」列を常に見えるように |
| P2 | **ドキュメント一覧ページング** | API は `limit`/`offset` のみ（デフォルト 100・`total` なし）。GUI 未対応。数千〜数万件で必須 |
| P2 | **ドキュメント検索（ソース内）** | ページングとセット。まずタイトル `LIKE`、必要ならプレビュー本文 |
